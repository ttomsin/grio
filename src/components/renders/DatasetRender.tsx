    ...
    <Card className="my-4 border-border bg-card">
      ...
      <CardContent>
        ...
        {schema && schema.length > 0 && (
          <div className="mb-6">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Schema</p>
            <div className="flex flex-wrap gap-2">
              {schema.map((field: string, i: number) => (
                <Badge key={i} variant="secondary" className="text-[10px] bg-background border border-border">
                  {field}
                </Badge>
              ))}
            </div>
        ...
        <div className="flex gap-3">
          {(format === 'json' || format === 'both') && (
            <Button onClick={() => handleDownload('json')} variant="outline" className="flex-1 border-border hover:bg-muted">
              <Download className="w-4 h-4 mr-2" /> JSON
            </Button>
          )}
          {(format === 'csv' || format === 'both') && (
            <Button onClick={() => handleDownload('csv')} variant="outline" className="flex-1 border-border hover:bg-muted">
